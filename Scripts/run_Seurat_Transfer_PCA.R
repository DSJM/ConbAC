...
run_Seurat_Transfer_PCA<-function(DataPath,LabelsPath,CV_RDataPath,OutputDir,GeneOrderPath = NULL,NumGenes = NULL){
  "
  run Seurat
  ...
  for (i in c(1:n_folds)){
    if(!is.null(GeneOrderPath) & !is.null (NumGenes)){
      ...
      ## Limited to the amount of selected features to reduce calculation time, might be increased or removed
      ...
      seur_Train@meta.data$Celltype = Labels[Train_Idx[[i]]]
      seur_Train <- NormalizeData(seur_Train)
      ## Limited to 5000 variable features to reduce calculation time, might be removed
  ...
  write.csv(True_Labels_Seurat,paste0(OutputDir,'/Seurat_PCA_true.csv'),row.names = FALSE)
  write.csv(Pred_Labels_Seurat,paste0(OutputDir,'/Seurat_PCA_pred.csv'),row.names = FALSE)
  write.csv(Total_Time_Seurat,paste0(OutputDir,'/Seurat_PCA_total_time.csv'),row.names = FALSE)
  }

if (args[6] == "0") {
  run_Seurat_Transfer_PCA(args[1], args[2], args[3], args[4])
} else {
  run_Seurat_Transfer_PCA(args[1], args[2], args[3], args[4], args[5], as.numeric(args[6]))
}